...
// layout and `nextra-dgmo/client.css` wiring untouched. CI-only: the rewritten
...
// ("Unexpected character `!` before name"). The upstream all-chart-types.md
// uses one such comment as an inline annotation; it renders to nothing anyway.
// Every other `<`/`{` in the upstream lives inside a ```dgmo fence, which MDX
// treats as opaque, so no further sanitizing is needed.
...
// Same MDX guard for the intro: it is a shared file in dgmo-content, and one
// HTML comment added there later would otherwise break this build and no other.
